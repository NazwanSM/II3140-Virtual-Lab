...
export default function LatihanInfoCard({
    ...
    return (
        <div 
            className={`bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 w-full max-w-sm relative ${
                isLocked 
                    ? 'opacity-60 cursor-not-allowed' 
                    : 'hover:shadow-xl hover:scale-105 cursor-pointer'
            }`}
            onClick={!isLocked ? onSoalClick : undefined}
        >
            {isLocked && (
            ...
            <div className="relative h-48 w-full">
                ...
                <div className="absolute top-4 left-4 bg-white rounded-full px-3 py-1 flex items-center gap-2 shadow-md">
                    <span className="text-sm font-bold text-gray-800">Soal {soalNumber}</span>
            ...
            <div className="p-5">
                ...
                {!isLocked && (
                    <button
                        className="w-full px-4 py-3 bg-linear-to-r from-[#5C6BC0] to-[#3949AB] hover:from-[#4A5AB8] hover:to-[#2838A0] rounded-xl text-sm font-bold text-white transition-all duration-200 shadow-md hover:shadow-lg"
                    >
                        Mulai Latihan
                    </button>
                )}
